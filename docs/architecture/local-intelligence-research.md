...
**Status: historical research record from 25 July 2026.** V1 local note enrichment is now implemented on `main`. This record also evaluates a deferred active-recall feature. See the current [local note-enrichment architecture](local-note-enrichment.md).

## Chosen first step

V1 autosaves a learner's note as a draft without analysis. **Finish lecture** marks the selected revision ready and starts a best-effort local response in the background. The atomic capture remains the source of truth; model output is a revision-keyed, separate artifact. The app works normally when the model is absent. No account login, cloud inference, model download, or automatic Ollama startup is part of this feature.
...
V1 now provides draft autosave, explicit Finish Lecture analysis, isolated model output, a revision-safe reader, queue coalescing, and explicit unavailable/failed states. Add compression only as a separate source-and-recall feature after real-note evaluation validates this loop.
