...
**Status: historical research record from 25 July 2026.** The extracted study-response experiment described here has been removed. This record remains only as design history; the current local AI surfaces are note organization, topic-note generation, and question-authoring assistance.
...
The current product keeps draft autosave, explicit Finish lecture organization, deterministic reading sections, topic-note generation, and review-gated distractor suggestions. Add compression only as a separate source-and-recall feature after real-note evaluation validates that loop.
