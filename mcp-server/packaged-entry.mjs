...
if (!process.env.REVEMBER_KNOWLEDGE_ROOT || !process.env.REVEMBER_PROGRESS_PATH) {
  try {
    ...
    if (settings && typeof settings === "object" && !Array.isArray(settings)) {
      if (!process.env.REVEMBER_KNOWLEDGE_ROOT && typeof settings.knowledgeRootPath === "string" && settings.knowledgeRootPath.trim()) {
        process.env.REVEMBER_KNOWLEDGE_ROOT = settings.knowledgeRootPath;
      }
      if (!process.env.REVEMBER_PROGRESS_PATH && typeof settings.progressPath === "string" && settings.progressPath.trim()) {
        process.env.REVEMBER_PROGRESS_PATH = settings.progressPath;
      }
    }
  } catch (error) {
