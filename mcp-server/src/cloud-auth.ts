...
export function authenticate(options: CloudOptions, token: string): CloudIdentity {
  try {
    const [version, nonce, encrypted, tag, extra] = token.split(".");
    if (version !== "rv1" || !nonce || !encrypted || !tag || extra || token.length > 16000) throw new Error();
    const decipher = createDecipheriv("aes-256-gcm", key(options), Buffer.from(nonce, "base64url"));
