...
export function CheckIn({ topic, snapshot, onSnapshot }: { topic: KnowledgeTopic; snapshot: AppSnapshot; onSnapshot: (snapshot: AppSnapshot) => void }) {
  ...
  const { choice, selectedChoiceID, rating, setRating, revealed, setRevealed, error, setError, choose } = useQuestionAttempt(question);

  useEffect(() => { setIndex(0); setSaved(undefined); }, [topic.id]);
  ...
  const move = (next: number) => setIndex(Math.max(0, Math.min(questions.length - 1, next)));
...
export function ReviewSession({ items, onSnapshot, onFinish }: { items: DueReviewItem[]; onSnapshot: (snapshot: AppSnapshot) => void; onFinish: () => void }) {
  ...
  const { choice, selectedChoiceID, rating, setRating, revealed, setRevealed, error, setError, choose } = useQuestionAttempt(item?.question);
  ...
  return <div className="review-shell"><div className="review-top"><button onClick={onFinish}><X /> Exit Review</button><span><Timer /> {index + 1} of {items.length}</span></div><section className="surface review-card"><div className="review-context"><div><Eyebrow>{item.isRevised ? "Revised Check" : item.isNew ? "New Check" : "Due Check"}</Eyebrow><strong>{item.topic.title}</strong></div><span>{capitalize(item.question.transferLevel)}</span></div>
    ...
    {error && <InlineError message={error} />}<div className="review-save"><span>The answer and rating are saved together.</span><button className="primary" disabled={!choice || !rating} onClick={save}>{index === items.length - 1 ? "Finish Review" : "Save & Continue"} <ArrowRight /></button></div>
  </section></div>;
}
