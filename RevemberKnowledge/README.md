...
The current renderer is intentionally focused on authored questions and review. It no longer exposes a learner-capture editor, note reader, topic-note generation, or manual concept/relationship management. Existing `captures/` and segmentation artifacts remain compatible with the local store, while question authoring can request editable local distractor suggestions; the learner must review and save the final question.
